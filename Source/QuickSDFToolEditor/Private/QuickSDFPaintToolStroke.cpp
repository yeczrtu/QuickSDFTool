...
bool UQuickSDFPaintTool::UpdateExternalViewportPointerHover(const FVector2D& AbsoluteScreenPosition)
{
	...
	SetTextureCanvasCursorActive(false);
...
bool UQuickSDFPaintTool::HandleExternalViewportPenPointer(const FVector2D& AbsoluteScreenPosition, float Pressure, bool bInContact)
{
	UpdateExternalPenPointerState(AbsoluteScreenPosition, Pressure, bInContact);

	FVector2D ViewportPosition;
	FRay WorldRay;
	const bool bHasViewportRay = TryResolveViewportPointerPosition(AbsoluteScreenPosition, ViewportPosition, WorldRay);
	if (bHasViewportRay)
	{
		LastExternalViewportPenDeviceRay = FInputDeviceRay(WorldRay, ViewportPosition);
	}

	if (!bInContact)
	{
		if (bExternalViewportPenStrokeActive)
		{
			OnClickRelease(LastExternalViewportPenDeviceRay);
			bExternalViewportPenStrokeActive = false;
			return true;
		}
		return false;
	}
	...
	if (!bHasViewportRay)
	{
		return bExternalViewportPenStrokeActive;
	}

	const FModifierKeysState ModifierKeys = FSlateApplication::Get().GetModifierKeys();
	SetShiftToggle(ModifierKeys.IsShiftDown());
	SetCtrlToggle(ModifierKeys.IsControlDown());

	if (bExternalViewportPenStrokeActive)
	{
		OnClickDrag(LastExternalViewportPenDeviceRay);
		return true;
	}

	const FInputRayHit Hit = CanBeginClickDragSequence(LastExternalViewportPenDeviceRay);
	if (!Hit.bHit)
	{
		return false;
	}

	OnClickPress(LastExternalViewportPenDeviceRay);
	bExternalViewportPenStrokeActive = true;
	return true;
}
...
bool UQuickSDFPaintTool::HitTest(const FRay& Ray, FHitResult& OutHit)
{
	if (CurrentComponent.IsValid())
	{
		...
		if (TargetMeshSpatial.IsValid() && TargetMesh.IsValid())
		{
			...
			const bool bUseTargetMaterialSlotFilter = Properties && Properties->TargetMaterialSlot >= 0;
			UE::Geometry::IMeshSpatial::FQueryOptions QueryOptions(100000.0, [this, bUseTargetMaterialSlotFilter](int32 TriangleID)
			{
				return !bUseTargetMaterialSlotFilter || IsTriangleInTargetMaterialSlot(TriangleID);
			});
...
bool UQuickSDFPaintTool::TryMakeStrokeSample(const FRay& Ray, FQuickSDFStrokeSample& OutSample)
{
	...
	const FTransform Transform = CurrentComponent->GetComponentTransform();
	const FRay LocalRay(Transform.InverseTransformPosition(Ray.Origin), Transform.InverseTransformVector(Ray.Direction));
	const bool bUseTargetMaterialSlotFilter = Properties->TargetMaterialSlot >= 0;
	UE::Geometry::IMeshSpatial::FQueryOptions QueryOptions(100000.0, [this, bUseTargetMaterialSlotFilter](int32 TriangleID)
	{
		return !bUseTargetMaterialSlotFilter || IsTriangleInTargetMaterialSlot(TriangleID);
	});
...
{
	if (bTextureCanvasCursorActive == bActive)
	{
		return;
	}
