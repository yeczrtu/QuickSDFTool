...
constexpr float QuickSDFTimelineSeekLaneHeight = 20.0f;
constexpr float QuickSDFTimelineKeyframeLaneHeight = 48.0f;
...
float GetQuickSDFTimelineTrackWidth(const TSharedPtr<SCanvas>& Canvas)
{
	if (!Canvas.IsValid())
	{
		return 0.0f;
	}
	return FMath::Max(0.0f, Canvas->GetTickSpaceGeometry().GetLocalSize().X - (QuickSDFTimelineTrackPadding * 2.0f));
}

float GetQuickSDFTimelineAngleX(const TSharedPtr<SCanvas>& Canvas, float Angle, float MaxAngle)
{
	return GetQuickSDFTimelineTrackWidth(Canvas) * QuickSDFTimelineStatus::NormalizeAngleToTimelinePercent(Angle, MaxAngle) + QuickSDFTimelineTrackPadding;
}
...
FText SQuickSDFTimeline::GetCompactSummaryText() const
{
	...
	const int32 CurrentIndex = Props ? FMath::Clamp(Props->EditAngleIndex, 0, FMath::Max(Props->NumAngles - 1, 0)) : 0;
	const float CurrentAngle = Props && Props->TargetAngles.IsValidIndex(CurrentIndex) ? Props->TargetAngles[CurrentIndex] : 0.0f;
	return FText::Format(LOCTEXT("CompactSummary", "{0} masks ready. Current {1}: {2} deg. Click or drag the upper rail to seek."), MaskCount, CurrentIndex + 1, FText::AsNumber(FMath::RoundToInt(CurrentAngle)));
}
...
int32 SQuickSDFTimeline::FindKeyframeAtScreenPosition(const FVector2D& ScreenPosition) const
{
	...
	for (int32 Index = 0; Index < Props->TargetAngles.Num(); ++Index)
	{
		...
		const float KeyX = FMath::Clamp(Angle / MaxAngle, 0.0f, 1.0f) * TrackWidth + QuickSDFTimelineTrackPadding;
...
void SQuickSDFTimeline::RebuildTimeline()
{
	...
	UQuickSDFToolProperties* Props = Tool->Properties;
	if (!Props) return;
	if (!Props->TargetAngles.IsValidIndex(HoveredOffsetKeyIndex))
	{
		HoveredOffsetKeyIndex = INDEX_NONE;
	}
	...
	[
		...
		.BorderImage(FAppStyle::GetBrush("WhiteBrush"))
		.BorderBackgroundColor(FLinearColor(0.018f, 0.018f, 0.018f, 0.96f))
	];
	...
	.Size(TAttribute<FVector2D>::CreateLambda([this]() {
		...
		return FVector2D(FMath::Max(1.0f, TrackWidth), QuickSDFTimelineRailHeight);
	}))
	[
		SNew(SBorder)
		.Visibility(EVisibility::HitTestInvisible)
	...
	.Size(TAttribute<FVector2D>::CreateLambda([this]() {
		...
		const float TrackWidth = TimelineTrackCanvas->GetTickSpaceGeometry().GetLocalSize().X - (QuickSDFTimelineTrackPadding * 2.0f);
		return FVector2D(FMath::Max(0.0f, TrackWidth) * Percent, QuickSDFTimelineRailHeight);
	}))
	[
		SNew(SBorder)
		.Visibility(EVisibility::HitTestInvisible)
		.BorderImage(FAppStyle::GetBrush("WhiteBrush"))
		.BorderBackgroundColor(GetQuickSDFTimelineAccentColor(0.75f))
	];
	...
	[
		...
		.Visibility(EVisibility::HitTestInvisible)
		.BorderImage(FAppStyle::GetBrush("WhiteBrush"))
		.BorderBackgroundColor(FLinearColor(0.018f, 0.018f, 0.018f, 0.98f))
	];
	...
	for (int32 i = 0; i < Props->NumAngles; ++i)
	{
		...
		.Position(TAttribute<FVector2D>::CreateLambda([this, i, KeyframeLaneTop]() {
			...
			float TrackWidth = TimelineTrackCanvas->GetTickSpaceGeometry().GetLocalSize().X - (QuickSDFTimelineTrackPadding * 2.0f);
	...
	for (int32 i = 0; i <= NumTicks; ++i)
	{
		...
		.Position(TAttribute<FVector2D>::CreateLambda([this, Percent]() {
			const float TrackWidth = TimelineTrackCanvas->GetTickSpaceGeometry().GetLocalSize().X - (QuickSDFTimelineTrackPadding * 2.0f);
			return FVector2D(FMath::Max(0.0f, TrackWidth) * Percent + QuickSDFTimelineTrackPadding - 0.5f, 4.0f);
		}))
		.Size(FVector2D(1.0f, 11.0f))
		[
			SNew(SBorder)
			.Visibility(EVisibility::HitTestInvisible)
			.BorderImage(FAppStyle::GetBrush("WhiteBrush"))
			.BorderBackgroundColor(FLinearColor(0.85f, 0.85f, 0.85f, 0.28f))
		];
	}
	...
	for (int32 OffsetKeyIndex = 0; OffsetKeyIndex < Props->TargetAngles.Num(); ++OffsetKeyIndex)
	{
		...
		.Position(TAttribute<FVector2D>::CreateLambda([this, OffsetKeyIndex]()
		{
			...
			const float MaxAngle = GetQuickSDFTimelineMaxAngle(P);
			if (P->UsesFrontHalfAngles() && P->TargetAngles[OffsetKeyIndex] > MaxAngle)
			{
				return FVector2D(-1000.0f, -1000.0f);
			}
		...
		[
			...
			.Visibility(TAttribute<EVisibility>::CreateLambda([this, OffsetKeyIndex]()
			{
				...
				return QuickSDFTimelineStatus::ShouldShowOffsetVisual(BuildTimelineKeyStatus(Tool, OffsetKeyIndex).AngleOffsetDelta)
					? EVisibility::HitTestInvisible
					: EVisibility::Collapsed;
			}))
			.BorderImage(FAppStyle::GetBrush("WhiteBrush"))
			.BorderBackgroundColor(TAttribute<FSlateColor>::CreateLambda([this, OffsetKeyIndex]()
			{
				const UQuickSDFPaintTool* Tool = GetActivePaintTool();
				const UQuickSDFToolProperties* P = Tool ? Tool->Properties : nullptr;
				const bool bStrong = P && (P->EditAngleIndex == OffsetKeyIndex || HoveredOffsetKeyIndex == OffsetKeyIndex);
				return FSlateColor(GetQuickSDFTimelineOffsetColor(bStrong ? 0.88f : 0.46f));
			}))
		];
		...
		[
			...
			.BorderBackgroundColor(TAttribute<FSlateColor>::CreateLambda([this, OffsetKeyIndex]()
			{
				...
				const UQuickSDFToolProperties* P = Tool ? Tool->Properties : nullptr;
				const bool bStrong = P && (P->EditAngleIndex == OffsetKeyIndex || HoveredOffsetKeyIndex == OffsetKeyIndex);
				return FSlateColor(GetQuickSDFTimelineOffsetColor(bStrong ? 0.98f : 0.64f));
			}))
		];

	}

	// 4. Add Keyframes in two passes to handle "Z-Order" (Active on top)
	auto AddKeyframeToCanvas = [this, Props, KeyframeLaneTop, KeyframeLaneHeight, KeyframeWidth](int32 i)
	{
		...
		.Position(TAttribute<FVector2D>::CreateLambda([this, i, KeyframeLaneTop, KeyframeWidth]() {
			...
			if (ActiveTool && ActiveTool->Properties && ActiveTool->Properties->TargetAngles.IsValidIndex(i))
			{
				...
				float TrackWidth = TimelineTrackCanvas->GetTickSpaceGeometry().GetLocalSize().X - (QuickSDFTimelineTrackPadding * 2.0f);
				return FVector2D(FMath::Max(0.0f, TrackWidth) * Percent + QuickSDFTimelineTrackPadding - (KeyframeWidth * 0.5f), KeyframeLaneTop);
			}
			return FVector2D::ZeroVector;
		}))
		...
		[
			...
			.OnHoverStarted(this, &SQuickSDFTimeline::OnKeyframeHovered, i)
			.OnHoverEnded(this, &SQuickSDFTimeline::OnKeyframeUnhovered, i)
	...
	.Position(TAttribute<FVector2D>::CreateLambda([this]() {
		...
		float TrackWidth = TimelineTrackCanvas->GetTickSpaceGeometry().GetLocalSize().X - (QuickSDFTimelineTrackPadding * 2.0f);
		return FVector2D(FMath::Max(0.0f, TrackWidth) * Percent + QuickSDFTimelineTrackPadding - 1.0f, 2.0f);
	}))
	...
	[
		...
		.Visibility(EVisibility::HitTestInvisible)
		.BorderImage(FAppStyle::GetBrush("WhiteBrush"))
		.BorderBackgroundColor(FLinearColor(1.0f, 0.9f, 0.28f, 0.9f))
	];
	...
	.Position(TAttribute<FVector2D>::CreateLambda([this]() {
		...
		float Percent = FMath::Clamp(SeekAngle / MaxAngle, 0.0f, 1.0f);
		float TrackWidth = TimelineTrackCanvas->GetTickSpaceGeometry().GetLocalSize().X - (QuickSDFTimelineTrackPadding * 2.0f);
		return FVector2D(FMath::Max(0.0f, TrackWidth) * Percent + QuickSDFTimelineTrackPadding - 3.0f, QuickSDFTimelineRailY - 2.0f);
	}))
	.Size(FVector2D(6.0f, 7.0f))
	[
		SNew(SBorder)
		.Visibility(EVisibility::HitTestInvisible)
		.BorderImage(FAppStyle::GetBrush("WhiteBrush"))
		.BorderBackgroundColor(FLinearColor(1.0f, 0.9f, 0.28f, 1.0f))
	];
}
...
void SQuickSDFTimeline::OnKeyframeHovered(int32 Index)
{
	if (HoveredOffsetKeyIndex == Index)
	{
		return;
	}

	HoveredOffsetKeyIndex = Index;
	Invalidate(EInvalidateWidgetReason::PaintAndVolatility);
}

void SQuickSDFTimeline::OnKeyframeUnhovered(int32 Index)
{
	if (HoveredOffsetKeyIndex != Index)
	{
		return;
