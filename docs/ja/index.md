...
<div class="doc-grid">
  <a class="doc-card" href="{{ '/ja/workflow/' | relative_url }}">
    ...
    <span>Select、Paint、2D Canvas、液タブ入力、Material Slots、Timeline、Live SDF preview、Symmetry、Guard、import/export、SDF generation を確認します。</span>
  </a>
...
8. 最終 texture 生成前の確認には **Live SDF** material preview を使います。GPU JFA による高速な近似表示です。
9. タイムラインでライト角度を移動し、キーフレーム追加や Paint Target を選びます。
10. **Generate Selected SDF** または **Generate SDF Threshold Map** を実行します。
