...
- 上段シークレーンと下段キーフレームレーンに分離した角度タイムライン。サムネイル、固定幅で見やすい角度ラベル、5 度スナップ、追加 / 複製 / 削除、シンメトリー対応のマスク補完、均等配置、`DirectionalLight` 同期に対応しています。
...
- **角度タイムライン UI:** シーク専用レーンとキーフレームレーンを分け、見やすいサムネイル区切り、固定幅の角度 / ステータス数値、ステータスバッジ、追加 / 複製 / 削除、スナップ、シンメトリー対応の補完、均等配置を提供します。
