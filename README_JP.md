...
| Select mode はメッシュ全体を表示したまま、選択行と cyan viewport overlay で active slot を示します。 | Paint mode は Screen projection、ビューポート上のブラシプレビュー、UV texture preview、選択中 material slot を同時に確認できます。 |
...
- メッシュ全体を表示したまま始まる Select / 準備ワークフロー。UE エディター選択と Quick SDF target を同期し、Static Mesh / Skeletal Mesh コンポーネントをビューポートから直接選択できます。
- Static Mesh / Skeletal Mesh コンポーネントへの直接ペイントと、Paint 中のマテリアルスロット分離、スロット単位のヒットテスト、ブラシ位置フォーカス。
- ビューポートクリックまたは行クリックで選択できるコンパクトな `Material Slots` リスト、Select 中の cyan active-slot overlay、`Selected` / `Baked` / `Empty` ステータスピル、選択スロットだけを Bake するスロット単位の Bake 操作。複数スロット一括 Bake は基本ワークフローから外しています。
- UV ガイドとオニオンスキンに対応した 2D UV プレビューペイント。
- 上段シークレーンと下段キーフレームレーンに分離した角度タイムライン。サムネイル、固定幅で見やすい角度ラベル、5 度スナップ、キーフレームドラッグとシークの同期、追加 / 複製 / 削除、シンメトリー対応のマスク補完、均等配置、`DirectionalLight` 同期に対応しています。
...
- `Auto`、テクスチャ全体の 0-90 反転、UV アイランド単位のチャンネル反転、0-180 通常ペイントを選べるシンメトリーモード。ホールド式クイックストローク、全角度ペイント系ワークフロー、スイープ範囲に応じた 8 マスク / 15 マスクの標準補完にも対応しています。
...
- Monopolar / Bipolar の自動パッキング、1x-8x アップスケール、シェーダー互換の R/A/B/G 出力スウィズル、half-float テクスチャ出力に対応した CPU SDF 生成。
...
4. Select mode で、編集したいメッシュ / マテリアル面をビューポート上でクリックします。メッシュを選択した状態でモードに入ることもできます。
5. **Material Slots** で active slot を確認します。選択行と cyan overlay が現在のマテリアルスロットを示し、行クリックでビューポート選択を補正できます。
...
8. タイムライン上段でライト角度をシークし、下段のキーフレームレーンで選択、追加、複製、削除、ドラッグ移動を行います。
9. 現在マスクのみ、全マスク、現在より前 / 後の範囲へストロークを反映したい場合は Paint Target モードを選びます。タイムラインのハイライト範囲が編集対象を示します。
10. ツール詳細の **Generate Selected SDF** または **Generate SDF Threshold Map** を実行します。
11. `/Game/QuickSDF_GENERATED/` に生成されたテクスチャをトゥーンマテリアルで使います。

詳しい流れは [Examples](./Examples/README.md)、[Material Setup](./docs/material-setup.md)、[Troubleshooting](./docs/troubleshooting.md) を参照してください。
...
| Select mode の `Viewport Mesh / Material Click` | target mesh component と、クリックした material slot を選択 |
...
| `Material Slot Row Click` | 編集するマテリアルスロット / Texture Set を選択、または補正 |
...
- **Select 準備ワークフロー:** まず非破壊の Select 状態で開始し、メッシュ全体を表示したまま、選択なし入場時は古い target を消し、ビューポートクリックで mesh component と material slot を同時に選べます。
- **メッシュ直接ペイント:** 対象メッシュ表面へリアルタイムプレビュー付きでペイントし、マテリアルスロット単位のフィルタリング、Paint 中の既定 slot isolation、`F` によるブラシ位置フォーカスも使えます。
- **Material Slot ワークフロー:** ビューポートクリックまたは行クリックでスロットを選択し、`Selected` / `Baked` / `Empty` の状態を小さなピルで確認し、Select 中は cyan overlay で active slot を表示し、行アクションからアクティブスロットだけを Bake します。トゥーン影マスク編集はスロット単位の作業が中心になるため、一括 Bake 系ボタンは標準 UI から外しています。
...
- **シンメトリーモード:** `Auto`、`Texture Flip`、`UV Island Channel Flip`、`Off` を選び、0-90 度だけをペイントするか、90-180 度側をどの方法で生成するかを制御できます。最終テクスチャは通常の 0-180 シェーダー経路でそのまま読める形式を維持します。
...
- **角度タイムライン UI:** シーク専用レーンとキーフレームレーンを分け、キーフレームドラッグとシークの同期、見やすいサムネイル区切り、固定幅の角度 / ステータス数値、ステータスバッジ、追加 / 複製 / 削除、スナップ、シンメトリー対応の補完、均等配置を提供します。
...
- **SDF 生成パイプライン:** SDF 補間、Monopolar / Bipolar の自動 RGBA パッキング、R/A/B/G 出力スウィズル、half-float テクスチャ出力で threshold map を生成します。UV アイランド単位反転では、Monopolar でもグレースケール単独ではなく RGBA16F / HDR 互換データとして出力します。
...
- `Auto`: 顔ペイント向けの既定モードです。対象メッシュ、UV チャンネル、マテリアルスロットを解析し、UV 占有がテクスチャ全体で左右対称なら `Texture Flip`、左右アイランドが分離している、曖昧、または 0-1 外 UV がある場合は `UV Island Channel Flip` を選びます。
- `Off`: 0-180 度の全範囲を通常通りペイントします。
- `Texture Flip`: 0-90 度だけをペイントし、90-180 度側はテクスチャ全体を反転して生成します。UV レイアウト全体が左右対称な場合に向いています。
- `UV Island Channel Flip`: 0-90 度だけをペイントし、90-180 度側は UV アイランド単位で生成します。ソースアイランドを island-local の左右反転でサンプルし、通常の 0-180 map と同じ最終 RGBA レイアウトへ書き込みます。

`UV Island Channel Flip` は、左右の UV アイランドが別々に配置されていても、形状としてはミラー関係にあるアセット向けです。位置、スケール、軽い形状差は normalized island-local 変換で吸収しますが、非線形の per-island ワープは行いません。曖昧なアイランド、未ペア、重なり、0-1 外 UV などは source 側の値コピーへフォールバックし、warning を残します。

生成テクスチャは通常の 0-180 レイアウトと同じシェーダー経路で読めます。内部の combined field は従来互換の `R/G/B/A` として扱い、最終出力時に R/A/B/G スウィズル (`R <- R`, `G <- A`, `B <- B`, `A <- G`) を行うことで、既存シェーダーの期待値と従来の `B` チャンネル挙動を維持します。
...
- Select mode では、ビューポート上のメッシュ面をクリックすると target mesh component とヒットした material slot が同時に選択されます。
- Select / 準備状態ではメッシュ全体を表示したまま、active slot を cyan overlay で示します。
- Paint mode は **Isolate Slot** が既定でオンの状態から始まります。オフにすると全体表示へ戻りますが、選択中スロットは引き続きペイント対象として扱われます。
- ペイントのピックとストロークサンプリングは選択中スロットを編集フィルターとして使うため、非対象スロットがアクティブスロットのヒットを奪いません。
- 各行にはスロット番号、スロット名、マテリアル名、`Selected` / `Baked` / `Empty` などのコンパクトなステータスピルを表示します。
...
- 表示行は現在の mesh component から毎回再構築されるため、target 切り替え後に古い `Missing` 行が active UI に残りません。
...
- キーフレームをドラッグするとシークカーソルとプレビューライトも即時更新されるため、移動中のサムネイル位置と現在角度が同期します。
...
- ヘッダー角度数値とキーフレーム角度ラベルは、数字部分だけに固定幅を持たせています。1 桁、2 桁、3 桁の角度に変わっても、ツールバーボタンやサムネイル配置が押し出されません。
...
- クイックトグル上の表示名は `Guard`、ショートカットは `Alt + 8` です。
...
- 開発用 Automation Test では、デフォルト角度、角度名パース、SDF エッジケース、チャンネルパッキング、UV アイランド反転適用、アセット移行、マスクインポートモデル検証、`TimelineRangeStatus`、`TimelineKeyStatus`、Monotonic Guard の挙動を確認します。
...
4. **合成:** Monopolar / Bipolar 出力を自動判定し、値を RGBA チャンネルへ格納します。
   - **Monopolar:** 対称的な影向け。選択中のシンメトリーモードが後半側を生成する場合は、0-90 / 90-180 の値を分けて保持します。
   - **Bipolar:** 非対称な影向け。内部の combined field で生成した値を、出力時に R/A/B/G へ並べ替えるため、最終テクスチャはシェーダーが期待するレイアウトを保ち、B チャンネルは 0-90 側の値のまま維持されます。
   - **UV Island Channel Flip:** 0-90 側の combined field から開始し、90-180 側のチャンネルを UV アイランド単位の反転サンプルで埋め、通常の 0-180 map と同じ RGBA16F / HDR テクスチャとして出力します。
...
- [docs/release-notes](./docs/release-notes/) にある対応するリリースノートを使ってリリースを作成する。

## 既知の制限

- UV レイアウトによって、ブラシサイズと実際のペイント範囲が一致しない場合があります。
- 液タブ / ペンタブでは、ペンを浮かせた状態の移動でブラシ位置が更新されず、次にペイントしたときに前回位置から意図しない線が伸びる場合があります。
