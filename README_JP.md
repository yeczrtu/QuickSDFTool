...
- 0-180 通常ペイント、全体テクスチャの 0-90 反転、UV アイランド単位のチャンネル反転を選べるシンメトリーモード。ホールド式クイックストローク、全角度ペイント系ワークフロー、スイープ範囲に応じた 8 マスク / 15 マスクの標準補完にも対応しています。
...
- **シンメトリーモード:** 通常の `None180` ペイント、テクスチャ全体を反転する `WholeTextureFlip90`、UV アイランド単位で 0-90 側から 90-180 側を生成する `UVIslandChannelFlip90` を選べます。最終テクスチャは通常の 0-180 シェーダー経路でそのまま読める形式を維持します。
...
QuickSDFTool の SDF 生成には 3 つのモードがあります。

- `None180`: 0-180 度の全範囲を通常通りペイントします。
- `WholeTextureFlip90`: 0-90 度だけをペイントし、90-180 度側はテクスチャ全体を反転して生成します。UV レイアウト全体が左右対称な場合に向いています。
- `UVIslandChannelFlip90`: 0-90 度だけをペイントし、90-180 度側は UV アイランド単位で生成します。ソースアイランドを island-local の左右反転でサンプルし、通常の 0-180 map と同じ最終 RGBA レイアウトへ書き込みます。

`UVIslandChannelFlip90` は、左右の UV アイランドが別々に配置されていても、形状としてはミラー関係にあるアセット向けです。位置、スケール、軽い形状差は normalized island-local 変換で吸収しますが、非線形の per-island ワープは行いません。曖昧なアイランド、未ペア、重なり、0-1 外 UV などは source 側の値コピーへフォールバックし、warning を残します。
