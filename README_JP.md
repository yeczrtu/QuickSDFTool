...
- マテリアルスロット分離では、非対象スロットを非表示にするだけでなく、ペイントのヒット判定やブラシ投影からも除外します。特定の部位だけをアイソレートしたとき、見えていないスロットの当たり判定に邪魔されずペイントできるようにします。
- アイソレート状態は単なるビューポート表示フィルターではなく、編集対象フィルターとして扱います。フルメッシュのジオメトリがカーソル下に残っていても、現在選択中のスロットへ確実に塗れるようにします。
- 複数のマテリアルスロットがあり、それぞれ UV が分かれているメッシュでは、全スロットを単純に自動ベイクしても意味がない場合があります。未使用または無関係なスロットまで焼くと重くなるため、無条件の `Auto Bake All Slots` は避けます。
- 必要なスロットだけを選んで個別にベイクできる、明示的なマテリアルスロット別ベイク範囲を追加します。使わないスロットをスキップでき、ベイク前にコストを把握できるようにします。
- UV アイランドやマテリアルスロットが有用な結合レイアウトを共有していない場合は、スロットごとのベイク結果または警告を分けて扱います。

#### スレッショルドマップ逆変換

- 完成したスレッショルドマップに対してライト角度を指定し、その角度のマスクを復元またはプレビューする逆変換ワークフローを追加します。
- 逆変換では、少なくともプレビュー、現在のマスクへの抽出、新規マスクへの抽出に対応し、完成済みスレッショルドマップの確認や修正に使えるようにします。
...
- `マテリアルスロット分離とベイク範囲` は、アイソレートしたマテリアルスロットを見えていないスロットのヒット判定に邪魔されずペイントでき、UV が分かれている場合は明示的に選んだマテリアルスロットだけをベイクできることを確認します。
- `スレッショルドマップ逆変換` は、角度を指定して完成済みスレッショルドマップからマスクをプレビューし、修正や再利用のために抽出できることを確認します。
