...
> [!NOTE]
> このセクションは今後実装するためのロードマップ要件です。現在のプレビュー版ではまだ利用できず、現時点の C++ API、`UQuickSDFAsset` 形式、Slate UI、ショートカットを変更するものではありません。

#### Monotonic Guard / クリッピングマスク

- `Monotonic Guard` は、同じ UV ピクセルが対象角度範囲内で複数回反転しないようにするペイント補助です。`黒 -> 白 -> 黒` または `白 -> 黒 -> 白` のような並びは、SDF スレッショルドのグラデーション破綻につながるため、防止または警告対象にします。
- 角度増加 + 白領域増加、角度増加 + 白領域減少、角度減少 + 白領域増加、角度減少 + 白領域減少の4パターンに対応します。
- `Clip Direction` の標準は `Auto` とします。`0-90` 度では角度が増えるほど白領域が増えやすい前提、`90-180` 度ではその反対方向の前提で扱います。手動指定として `White Expands` と `White Shrinks` も用意する想定です。
- 既存の `Current / All / Before / After` ペイント対象モードと連動し、選択中のマスク範囲だけにクリッピング制約を適用します。
...
- `0 / 45 / 90` 度の3マスクで白領域が増える場合、ガード対象ピクセルは範囲内で一度だけ変化します。
- `90 / 135 / 180` 度の3マスクで白領域が減る場合も、反転した増減方向で同じ単調性を保証します。
