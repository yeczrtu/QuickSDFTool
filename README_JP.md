...
> [!IMPORTANT]
> 本プロジェクトはプロトタイプです。以下のリストは現在のソースツリーと照合し、実装済みの項目と残作業を分けています。

### 実装済み

- [x] **Skeletal Mesh サポート** — `FQuickSDFMeshComponentAdapter` が Static Mesh と Skeletal Mesh の両方をサポート
- [x] **既存テクスチャインポート** — 選択中のテクスチャまたは画像ファイルをマスク / キーフレーム素材としてインポート可能
- [x] **タイムライン均等配置** — 現在のキーフレームを有効なシンメトリ範囲内で均等配置
- [x] **エクスポート名 & 上書き設定** — SDF / マスクテクスチャ名と既存アセットの上書き可否を設定可能
- [x] **自動保存後のオーバーレイ復帰** — エディターの保存 / 自動保存後にタイムラインオーバーレイを再アタッチ

### コア生成

- [ ] **GPU 加速 SDF** — JFA Compute Shader と C++ バインディングは存在するが、`GenerateSDF()` はまだ CPU 側の `FSDFProcessor` 経路を使用
- [ ] **パフォーマンスプロファイリング** — 高解像度テクスチャ（4K+）ワークフローのベンチマークと最適化

### ペイント & タイムラインワークフロー

- [ ] **カスタムブラシ形状** — カスタムブラシアルファテクスチャのインポート対応
- [ ] **筆圧感知** — タブレットの筆圧によるブラシ不透明度とサイズ制御
- [ ] **フレーム複製** — 現在のタイムラインフレームとマスクデータを複製する明示的なコマンドを追加
- [ ] **タイムライン移動** — 前後のフレームへ移動するボタンとショートカットを追加

### アセット & データ管理

- [ ] **自動保存 / ホットリロード** — ペイントデータを定期的にチェックポイント保存し、未保存のマスク変更をリロード後に復元
- [ ] **マルチ UV チャンネルプレビュー** — アクティブな UV チャンネル選択は可能だが、複数チャンネルの同時可視化は未実装

### ドキュメント & リリース

- [ ] **ドキュメント & チュートリアル** — ビデオウォークスルーと詳細な Wiki を追加
- [ ] **アイコン & ブランディング** — カスタムエディターモードアイコンとプラグインブランディングアセットを作成
- [ ] **GitHub ソーシャルプレビュー** — GitHub リポジトリ共有用のソーシャルプレビュー画像を作成

### 既知の不具合

- UV マップの展開方法によって、ブラシサイズと実際にペイントされる範囲が一致しない場合があります。
- タイムライン画像をクリックした後、表示位置がずれる場合があります。
- SDF 出力の方向が反転している可能性があります。
