...
- Static Mesh / Skeletal Mesh コンポーネントへの直接ペイントと、対象マテリアルスロットの分離。
- 行クリックで選択できるコンパクトな `Material Slots` リスト、`Baked` / `Empty` ステータスピル、スロット単位の Bake 操作。複数スロット一括 Bake は基本ワークフローから外しています。
...
- 行右端のアクションボタンはそのスロットだけを Bake します。以前の `Bake Selected`、`Bake Missing`、`Generate All` のような一括操作は基本 UI から外しています。
...
- 現在の UI は、行クリックによるマテリアルスロット選択とスロット単位 Bake に対応済みです。今後は広い一括 Bake 操作を戻すのではなく、高度な編集範囲制御に集中します。
- マテリアルスロット分離時に、非対象スロットが非表示のまま paint hit を奪わないようにします。1 つのスロットや部位を isolate したとき、paint picking や brush projection は表示 / 編集対象ではないスロットの collision や hit を無視する必要があります。
- isolate 状態を単なるビューポート表示フィルターではなく編集対象フィルターとして扱い、フルメッシュのジオメトリがカーソル下に残っていても、現在選択中のスロットへ確実に塗れるようにします。
- UV レイアウトが分かれている複数スロットのメッシュ向けに、必要なスロットだけを選び、未使用スロットをスキップし、Bake 前にコストを把握できる任意の高度な Bake Scope 制御を追加します。
- UV アイランドやマテリアルスロットが有用な結合レイアウトを共有していない場合は、スロットごとの Bake 結果または警告を分けて扱います。
...
- `Material Slot Isolation and Bake Scope` は、既存のスロットリストを拡張し、非表示スロットが hit detection を奪わないことと、分離 UV レイアウト向けに任意の高度な Bake Scope 選択を追加することを確認します。
...
- [リリースノート](./Docs/ReleaseNotes/v0.1.0-preview.md) を使って `v0.1.0-preview` を作成する。
